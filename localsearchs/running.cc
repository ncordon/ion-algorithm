...
//    assert(fit >= 0);
    ...
    if(m_neval%(m_maxeval/10) == 0)
    {
        printf("#:%d:%.10e\n",m_neval,m_best);
    }
 
    //printf("#:%d:%e:%e\n",m_generalEval,fit,(double)m_generalBest);
    if (m_checkOptime->isBetter(fit, m_generalBest) ) {
	m_generalBest = fit;
        /*if(m_getConvergence){
          printf("#:%d:%e\n",m_generalEval,fit);
          if (convergence_file !=NULL)
                fprintf(convergence_file,"%d:%.10f\n",m_generalEval,fit);
        }*/
    }        
